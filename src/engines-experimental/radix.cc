...
status radix::iterate(typename container_type::iterator first,
		      ...
		      void *arg)
{
	for (auto it = first; it != last; ++it) {
		...
		auto ret = callback(it.key().data(), it.key().size(),
				    it.value().data(), it.value().size(), arg);
...
status radix::get(string_view key, get_v_callback *callback, void *arg)
{
	...
	if (it != container->end()) {
		...
		callback(it.value().data(), it.value().size(), arg);
		return status::OK;
	}
