...
vcmap_dram_locks::vcmap_dram_locks(std::unique_ptr<internal::config> cfg)
...
vcmap_dram_locks::~vcmap_dram_locks()
{
	LOG("Stopped ok");
}

std::string vcmap_dram_locks::name()
{
	return "vcmap_dram_locks";
}

status vcmap_dram_locks::count_all(std::size_t &cnt)
{
	LOG("count_all");
...
status vcmap_dram_locks::get_all(get_kv_callback *callback, void *arg)
{
	LOG("get_all");
...
status vcmap_dram_locks::exists(string_view key)
{
...
status vcmap_dram_locks::get(string_view key, get_v_callback *callback, void *arg)
{
...
status vcmap_dram_locks::put(string_view key, string_view value)
{
...
status vcmap_dram_locks::remove(string_view key)
{
...
internal::iterator_base *vcmap_dram_locks::new_iterator()
{
	return new vcmap_dram_locks_iterator<false>{&pmem_kv_container, &ch_allocator};
}

internal::iterator_base *vcmap_dram_locks::new_const_iterator()
{
	return new vcmap_dram_locks_iterator<true>{&pmem_kv_container, &ch_allocator};
}

vcmap_dram_locks::vcmap_dram_locks_iterator<true>::vcmap_dram_locks_iterator(
	container_type *c, ch_allocator_t *ca)
    : container(c), ch_allocator(ca)
{
}

vcmap_dram_locks::vcmap_dram_locks_iterator<false>::vcmap_dram_locks_iterator(
	container_type *c, ch_allocator_t *ca)
    : vcmap_dram_locks::vcmap_dram_locks_iterator<true>(c, ca)
{
}

status vcmap_dram_locks::vcmap_dram_locks_iterator<true>::seek(string_view key)
{
	init_seek();
...
result<string_view> vcmap_dram_locks::vcmap_dram_locks_iterator<true>::key()
{
	assert(!acc_.empty());
...
vcmap_dram_locks::vcmap_dram_locks_iterator<true>::read_range(size_t pos, size_t n)
{
	assert(!acc_.empty());
...
vcmap_dram_locks::vcmap_dram_locks_iterator<false>::write_range(size_t pos, size_t n)
{
	assert(!acc_.empty());
...
status vcmap_dram_locks::vcmap_dram_locks_iterator<false>::commit()
{
	for (auto &p : log) {
...
void vcmap_dram_locks::vcmap_dram_locks_iterator<false>::abort()
{
	log.clear();
}
