...
cmap_8_8::cmap_8_8(std::unique_ptr<internal::config> cfg)
    : pmemobj_engine_base(cfg, "pmemkv_cmap_8_8")
{
	LOG("Started ok");
	Recover();
}
...
void cmap_8_8::Recover()
{
	if (!OID_IS_NULL(*root_oid)) {
		container =
			(pmem::kv::internal::cmap_8_8::map_t *)pmemobj_direct(*root_oid);
		container->runtime_initialize();
	} else {
		pmem::obj::transaction::run(pmpool, [&] {
			pmem::obj::transaction::snapshot(root_oid);
			*root_oid =
				pmem::obj::make_persistent<internal::cmap_8_8::map_t>()
					.raw();
