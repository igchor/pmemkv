...
public:
    ...
    public:
        scoped_lock() {}
        scoped_lock( null_rw_mutex& , bool w = true ) {is_writer = w;}
        ~scoped_lock() {}
        void acquire( null_rw_mutex& , bool w = true ) {is_writer = w;}
        bool upgrade_to_writer() { is_writer = true; return true; }
        bool downgrade_to_reader() { is_writer = false; return true; }
        bool try_acquire( null_rw_mutex& , bool w = true ) { is_writer = w; return true; }
        void release() {}
...
using map_t = pmem::obj::concurrent_hash_map<uint64_t, obj::p<uint64_t>, std::hash<int>, std::equal_to<int>, null_rw_mutex, null_rw_mutex::scoped_lock>;
